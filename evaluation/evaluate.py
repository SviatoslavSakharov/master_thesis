...
def check_qvina_mols(qvina_scores, processed_mol):
    assert len(qvina_scores) == len(
        processed_mol
    ), f"len(qvina_scores) = {len(qvina_scores)}, len(processed_mol) = {len(processed_mol)}"
...
def evaluate_folder(args):
    ...
    all_qed, all_sa, all_logp, all_lipinski, per_pocket_diversity = mol_metrics.evaluate(
        processed_mol_filt, position=i, desc=basedir.name
    )
    ...
    with open(file, "w") as f:
        f.write(
            f"{sum([len(p) for p in processed_mol_filt])} molecules from {len(processed_mol_filt)} pockets evaluated. \n"
        )
